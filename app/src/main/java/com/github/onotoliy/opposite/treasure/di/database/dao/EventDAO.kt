...
interface EventDAO: WriteDAO<EventVO> {
    ...
    @Query("SELECT * FROM treasure_event ORDER BY milliseconds DESC LIMIT :offset, :numberOfRows")
    override fun getAll(offset: Int, numberOfRows: Int): LiveData<List<EventVO>>

    @Query("SELECT * FROM treasure_event ORDER BY milliseconds DESC")
    fun getAll(): LiveData<List<EventVO>>

    @Query("SELECT * FROM treasure_event WHERE name LIKE :name ORDER BY milliseconds DESC")
